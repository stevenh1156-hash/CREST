...
public static class LauncherEx
{
    ...
    {
        // BLSE feature patches -- when called via Bannerlord.BLSE.LauncherEx.exe,
        // these need to fire here because the standalone exe doesn't invoke
    ...
    /// <summary>
    /// CREST extraction of the BUTR-launcher setup portion of <see cref="Launch"/>.
    /// Registers all the Manager.Enable patches (BUTR mod-list mixin, hide-stubs,
    /// etc.) and primes the game-Main transpile, but does NOT invoke the launcher
    /// GUI and does NOT re-register the BLSE feature patches (which are
    /// registered separately -- by <see cref="Launch"/> when running inside
    /// Bannerlord.BLSE.LauncherEx.exe, or by <c>Shared.AppDomainManager.Initialize</c>
    /// when running inside the original launcher.exe via the .exe.config
    /// AppDomainManager redirect).
    ///
    /// Called from <c>Bannerlord.BLSE.Loaders.AppDomainManager.BLSEAppDomainManager</c>
    /// when the host process is the original launcher.exe -- so Steam-launch
    /// renders the BUTR-enhanced launcher (BLSE version label, hidden stubs)
    /// without the user needing to swap exes.
    /// </summary>
    public static void SetupBUTRPipeline(string[] args)
    {
        Manager.Initialize();
