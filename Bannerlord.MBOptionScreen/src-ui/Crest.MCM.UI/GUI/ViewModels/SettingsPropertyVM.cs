...
namespace MCM.UI.GUI.ViewModels
{
    internal sealed partial class SettingsPropertyVM : ViewModel
    {
        ...
        // Phase W.16 (CREST extension): callback hook that lets a settings
        // class gate the visibility of an individual property based on
        // arbitrary runtime conditions (e.g. another property's current
        // value). The hook is called with the current property's display
        // name and the parent settings instance; returning false hides the
        // property in the UI. Set to null when no per-property hiding is
        // needed (default).
        //
        // Used by CREST's CrestSettings to hide the campaign-log filter
        // bucket toggles when LogFilterScope != "None" (where they have
        // no effect anyway).
        public static Func<string /*displayName*/, BaseSettings /*owner*/, bool>? IsPropertyVisibleHook { get; set; }
        ...
        public bool IsSettingVisible
        {
            get
            {
                ...
                if (!SatisfiesSearch)
                    return false;
                // Phase W.16: external visibility hook. Settings classes
                // can register a callback to hide individual properties
                // based on the current value of other properties.
                if (IsPropertyVisibleHook != null)
                {
                    try
                    {
                        var owner = SettingsVM.SettingsInstance;
                        if (owner != null && !IsPropertyVisibleHook(SettingPropertyDefinition.DisplayName, owner))
                            return false;
                    }
                    catch { /* never throw from a getter */ }
                }
                return true;
            }
        }
