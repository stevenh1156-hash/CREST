...
namespace MCM.UI.GUI.ViewModels
{
    internal sealed class ModOptionsVM : ViewModel
    {
        ...
        // Phase W.18 (CREST): static reference to the most-recently constructed
        // ModOptionsVM so external code can ask the active mod-options page to
        // re-evaluate visibility on every property after a setting changes
        // (e.g. after the user changes a dropdown that gates which other
        // settings should be shown). Set in the constructor; cleared on
        // disposal. May be null if the page hasn't been opened this session.
        public static ModOptionsVM? Current { get; private set; }

        /// <summary>
        /// Phase W.18: trigger visibility refresh on the currently displayed
        /// mod-options page. Calls <c>NotifySearchChanged()</c> on every
        /// settings group of the currently selected entry, which causes each
        /// <see cref="SettingsPropertyVM.IsSettingVisible"/> to be re-queried.
        /// Safe to call when no page is open (no-op).
        /// </summary>
        public static void RefreshVisibility()
        {
            ...
            {
                var current = Current;
                if (current?.SelectedEntry?.SettingsVM == null) return;
                foreach (var group in current.SelectedEntry.SettingsVM.SettingPropertyGroups)
                    group.NotifySearchChanged();
            }
            catch { /* never throw from a UI helper */ }
        }
        ...
        /// <summary>
        /// CREST Phase N.3: returns true for the three internal CREST-bundled
        /// settings IDs that should be hidden from the in-game Mod Options list.
        /// See <see cref="InitializeModSettings"/> for the full rationale.
        /// </summary>
        private static bool IsCrestInternalSettingsId(string settingsId)
        {
            if (string.IsNullOrEmpty(settingsId)) return false;
            return settingsId == "MCM_v5"
                || settingsId == "MCMUI_v4"
                || settingsId == "Options";   // ButterLibSettingsContainer's id
        }
                ...
                try
                {
                    if (syncContext is SynchronizationContext uiContext)
                    {
                        ...
                        var settingsVM = BaseSettingsProvider.Instance?.SettingsDefinitions
                            // CREST Phase N.3: hide three internal CREST-bundled settings
                            // entries from the in-game Mod Options list. The underlying
                            // singletons remain registered so existing code that reads e.g.
                            // MCMUISettings.Instance still works -- this is a UI-list filter only.
                            //   - MCM_v5  : MCMSettings (no user-relevant settings)
                            //   - MCMUI_v4: MCMUISettings ("Hide Main Menu Entry" -- defaults work)
                            //   - Options : ButterLibSettingsContainer (UI is cosmetic;
                            //               ButterLib reads from Configs\ModSettings\ButterLib\
                            //               Options.json, not MCM's storage, so the controls
                            //               never affected behavior anyway)
                            .Where(s => !IsCrestInternalSettingsId(s.SettingsId))
                            .Parallel()
                            .Select(s =>
                            {
                                try
