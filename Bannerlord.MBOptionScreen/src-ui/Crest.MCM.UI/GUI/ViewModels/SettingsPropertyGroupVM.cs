...
namespace MCM.UI.GUI.ViewModels
{
    internal sealed class SettingsPropertyGroupVM : ViewModel
    {
        ...
        public void NotifySearchChanged()
        {
            ...
            {
                // Phase W.21: TaleWorlds Gauntlet binding system updates the
                // bound widget property only when OnPropertyChangedWithValue
                // is called -- bare OnPropertyChanged(name) raises the typed
                // event but the cached binding doesn't pick up the new value.
                // Calling both ensures search-text-driven AND scope-dropdown-
                // driven visibility changes propagate to IsVisible="@IsSettingVisible".
                prop.OnPropertyChanged(nameof(SettingsPropertyVM.IsSettingVisible));
                prop.OnPropertyChangedWithValue(prop.IsSettingVisible, nameof(SettingsPropertyVM.IsSettingVisible));
            }
            OnPropertyChanged(nameof(IsGroupVisible));
            OnPropertyChangedWithValue(IsGroupVisible, nameof(IsGroupVisible));
        }
