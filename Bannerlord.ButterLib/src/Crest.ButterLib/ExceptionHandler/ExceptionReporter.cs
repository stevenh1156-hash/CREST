...
public static class ExceptionReporter
{
    ...
    public static void Show(Exception exception)
    {
        ...
        // CREST: mirror the exception to runtime.log so the user (and Claude
        // reading Crest-Diag output) sees what blew up without having to
        // open the BEW popup and copy-paste the stack trace. Reflection into
        // Crest.Harmony.CrestDiag (loaded earlier in the SubModule chain)
        // keeps this fork compile-time-independent of Crest.Harmony's
        // exposed surface, mirroring how CrestConfig is reached from this
        // assembly elsewhere.
        try
        {
            ...
            if (logMethod != null)
            {
                var top = exception.StackTrace?.Split('\n').FirstOrDefault()?.Trim() ?? "(no stacktrace)";
                var inner = exception.InnerException is { } ie
                    ? $" / inner={ie.GetType().Name} {ie.Message}"
                    : "";
                logMethod.Invoke(null, new object[]
                {
                    "BEW",
                    $"{exception.GetType().FullName}: {exception.Message}{inner} | top={top}"
                });
            }
        }
        catch
        {
            // never let mirror-logging failure block the popup.
        }
